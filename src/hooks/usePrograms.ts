...
import { type IProgram, type IProgramForList } from '@/types'
...
export function usePrograms () {
  ...
  const [programsList, setProgramsList] = useState<IProgramForList[]>([])
  ...
  const getListPrograms = async (
    ...
    try {
      ...
      setProgramsList(convertToList(response.results))
      setLoading(false)
    } catch (error) {
      setLoading(false)
      setError(true)
